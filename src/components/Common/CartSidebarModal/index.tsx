...
const CartSidebarModal = () => {
  ...
  return (
    <div
      className={`fixed inset-0 z-[99999] overflow-hidden bg-dark/70 transition-all duration-300 ${isCartModalOpen ? "opacity-100 visible" : "opacity-0 invisible"
        }`}
    >
      <div
        className={`flex items-end sm:items-center justify-end h-full transition-transform duration-300 ${isCartModalOpen ? "translate-y-0 sm:translate-x-0" : "translate-y-full sm:translate-y-0 sm:translate-x-full"}`}
      >
        <div className="w-full h-[90vh] sm:h-screen sm:max-w-[500px] rounded-t-2xl sm:rounded-none shadow-1 bg-white px-4 sm:px-7.5 lg:px-11 relative modal-content flex flex-col">
          ...
          <div className="border-t border-gray-3 bg-white pt-5 pb-6 sm:pb-7.5 lg:pb-11 mt-auto sticky bottom-0">
                ...
                onClick={() => {
                  ...
                  cartItems.forEach((item) => {
                    if (item.affiliate_link) {
                      ...
                      if (item.id) {
                        tracking.trackAffiliateClick(item.id as string);
                      }
                      opened++;
                    }
                  });
                  if (opened === 0 && cartItems.length > 0) {
                    alert("No affiliate links found for these items.");
                  }
                  closeCartModal();
                }}
                disabled={cartItems.length === 0}
                className="w-full flex justify-center font-medium text-white bg-dark py-[13px] px-6 rounded-md ease-out duration-200 hover:bg-opacity-95 disabled:opacity-60"
              >
                Buy All Items
              </button>
            </div>

...
const Dropdown = ({ menuItem, stickyMenu }) => {
  ...
  return (
    <li
      ...
      className={`group relative before:w-0 before:h-[2px] before:bg-dark before:absolute before:left-0 before:top-0 before:rounded-b-[2px] before:ease-out before:duration-200 hover:before:w-full ${pathUrl.includes(menuItem.path) && "before:!w-full"
        }`}
    >
      <a
        href="#"
      ...
      >
        {menuItem.submenu.map((item, i) => (
          <li key={i}>
            <Link
              href={item.path}
              className={`flex text-custom-sm hover:text-dark hover:bg-gray-1 py-[7px] px-4.5 ${pathUrl === item.path && "text-dark font-medium bg-gray-1"
                } `}
            >
              {item.title}
            </Link>

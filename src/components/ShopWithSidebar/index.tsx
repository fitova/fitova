...
const ShopWithSidebar = () => {
  ...
  // Hook into StyleHub context to get the admin-managed colors
  const { filterOptions } = useStyleHub();
  const colorOptions = filterOptions.filter((o) => o.category === "color");
      ...
      <section className="overflow-hidden relative pb-20 pt-5 lg:pt-20 xl:pt-28 bg-[#f3f4f6]">
        <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0">
          ...
          <div className="xl:hidden flex justify-between items-center mb-6">
            <h2 className="text-xl font-playfair font-normal text-[#0A0A0A]">Shop All</h2>
            <button
              onClick={() => setProductSidebar(true)}
              className="flex items-center gap-2 bg-white border border-[#E8E4DF] px-4 py-2 text-sm font-medium text-[#0A0A0A] hover:bg-[#FAFAF9]"
            >
          ...
          <div className="flex gap-7.5 relative">
            ...
            {productSidebar && (
              <div
                className="fixed inset-0 bg-black/40 z-40 xl:hidden ease-out duration-300"
            ...
            {/* <!-- Sidebar Start --> */}
            <div
              className={`sidebar-content fixed xl:z-1 z-50 left-0 top-0 xl:translate-x-0 xl:static max-w-[310px] xl:max-w-[270px] w-[85%] sm:w-[350px] xl:w-full ease-out duration-300 ${productSidebar
                ? "translate-x-0 bg-white shadow-xl h-full overflow-y-auto border-r border-[#E8E4DF]"
                : "-translate-x-full"
                }`}
            >
              {/* Mobile Drawer Header */}
              <div className="xl:hidden flex items-center justify-between p-5 border-b border-[#E8E4DF] sticky top-0 bg-white z-10">
                <span className="font-playfair text-lg text-[#0A0A0A]">Filters</span>
                <button
                  onClick={() => setProductSidebar(false)}
                  className="p-2 -mr-2 text-[#8A8A8A] hover:text-[#0A0A0A]"
                >
              ...
              <div className="p-5 xl:p-0">
                <div className="flex flex-col gap-13">
                  {/* Header that has "Clean All" */}
                  <div className="flex items-center justify-between xl:pt-2">
                    <p className="text-[#0A0A0A] font-medium hidden xl:block">Filters:</p>
                    <button className="text-blue text-sm hover:underline">Clean All</button>
                  </div>
                  ...
                  {/* Aesthetic Color Wheel Filter */}
                  <div className="bg-white shadow-sm border border-[#E8E4DF] rounded-lg p-4 sm:p-5">
                    <h4 className="font-medium text-[#0A0A0A] flex items-center gap-2 mb-4">
                      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                        <path d="M12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2C6.47715 2 2 6.47715 2 12C2 13.5997 2.46279 15.0871 3.25056 16.3262C3.89973 17.3468 4.39088 18.5143 4.14924 19.7226C3.9076 20.9309 4.8876 22 6.12056 22H12Z" />
                      </svg>
                      Color Palette
                    </h4>

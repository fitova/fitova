...
const SingleItem = ({ item }: { item: Product }) => {
  ...
  return (
    <div className="group">
      <div className="relative overflow-hidden rounded-none border border-[#E8E4DF] transition-colors duration-300 hover:border-dark" style={{ backgroundColor: '#F6F5F2', minHeight: '403px' }}>
        <div className="text-center px-4 py-7.5">
          ...
          <h3 className="font-light tracking-wide text-dark ease-out duration-200 hover:text-dark-2 mb-1.5">
            <Link href="/shop-details"> {item.title} </Link>
          </h3>

          <span className="flex items-center justify-center gap-2 font-light text-lg">
            <span className="text-dark" style={{ fontWeight: 400 }}>${item.discountedPrice}</span>
            <span className="text-dark-4 line-through text-base">${item.price}</span>
          </span>
        </div>

        <div className="flex justify-center items-center">
          <Image src={item?.imgs?.previews?.[0] || "/images/products/product-1-bg-1.png"} alt={item?.title || "Best Seller"} width={280} height={280} />
        </div>
        ...
        <div className="absolute right-0 bottom-0 translate-x-full u-w-full flex flex-col gap-2 p-5.5 ease-linear duration-300 group-hover:translate-x-0">
          <button
            ...
            id="bestOne"
            className="flex items-center justify-center w-10 h-10 rounded-none border-t border-l border-[#E8E4DF] ease-out duration-300 text-dark bg-white hover:text-white hover:bg-dark"
          >
            <svg
